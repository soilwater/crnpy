...
def format_dates(source, col='timestamp', format='%Y-%m-%d %H:%M:%S', freq='H', round_time=True):
...
def add_count_time(source, k, col='timestamp'):
    """Approximate counting time for each detector based on timestamp
    """

    for k in range(len(cols_count)):
        cols_time_name = 'count_time_' + str(k+1) # 1-indexed
        source[cols_time_name] = source['timestamp'].diff().dt.total_seconds()
        
    return source
            
def fill_counts(source, cols_count, cols_time, count_time=3600, threshold=0.25):
...
def compute_total_raw_counts(source, cols_counts):
...
def is_outlier(source, cols_counts, window=11, store_outliers=False):
    ...
    """
    for k,col in enumerate(cols_counts):
        median = source[col].rolling(window, center=True, min_periods=1).median()
        AD = (source[col] - median).abs() # Absolute difference
        MAD = AD.median() # Median Absolute Difference
        M = 0.6745 * AD/MAD
        idx_outlier = M > 3
        source.loc[idx_outlier,col] = np.nan
        
        if store_outliers:
            outlier_col_name = 'outliers_'+str(k+1)
            source[outlier_col_name] = False
            source.loc[idx_outlier,outlier_col_name] = True
 
    return source
    
    
def fill_missing_atm(source, cols_atm,limit=24):
    ...
    for col in cols_atm:
        source[col].interpolate(method='pchip', limit_area='inside', limit=limit, inplace=True)
    
    return source


def atm_correction(source, cols_atm, par):
